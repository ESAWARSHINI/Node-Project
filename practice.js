...
//   const obj = movies.find((data) => data.id == id);
...
app.get("/movies", function (request, response) {
  ...
  if (request.query?.name) {
    const obj1 = movies.find((data) => data.name == request.query.name);
    obj1 ? response.send(obj1) : response.status(404).send(NOT_FOUND_MSG);
  } else {
    // or if( 'name' in request.params) { }
    response.send(movies);
  }
});
